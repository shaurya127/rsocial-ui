...
class _Search_PageState extends State<Search_Page> with TickerProviderStateMixin{
    ...
    if(allUsers.isNotEmpty)
      {
        for(int i=0;i<allUsers.length;i++)
        {
          Request_Tile tile = Request_Tile(
              user: allUsers[i],text: "Add",request: false,photourl:widget.photourl,curUser: widget.curUser,);
  ...
  buildSuggestedTab()
  {
    ...
    if(connections.isNotEmpty)
    {
      ...
      {
        Request_Tile tile = Request_Tile(
          user: connections[i],text: "Remove",request: false,photourl:widget.photourl,curUser: widget.curUser,);
  ...
  buildRequestTab()
  {
    ...
    if(requestList.isNotEmpty)
    {
      ...
      {
        Request_Tile tile = Request_Tile(
          user:requestList[i],request: true,photourl: requestList[i].photoUrl,text: "Accept",curUser: widget.curUser,);
        tiles.add(tile);
      }
      return ListView(
          children: tiles
  ...
  Widget BuildScreen(String Orientation)
  {
    if(Orientation=='request')
      return isLoading? Center(child: CircularProgressIndicator(),):buildRequestTab();

    else if(Orientation=="suggest")
      return isLoading? Center(child: CircularProgressIndicator(),):buildSuggestedTab();

    else if(Orientation=="search")
      return isLoading? Center(child: CircularProgressIndicator(),):buildSearchTab();
  }
...
class Request_Tile extends StatefulWidget {
  ...
  User user;
  User curUser;
  Request_Tile({this.request,this.text,this.user,this.photourl,this.curUser});
  @override
...
class _Request_TileState extends State<Request_Tile> {
  ...
  {
    var url = "https://9dhzla746i.execute-api.ap-south-1.amazonaws.com/user/removeconnection";
    var user = await FirebaseAuth.instance.currentUser();
    DocumentSnapshot doc = await users.document(user.uid).get();
    var uid = doc['id'];
    //print(uid);
  ...
  {
    var url = "https://9dhzla746i.execute-api.ap-south-1.amazonaws.com/user/addconnection";
    var user = await FirebaseAuth.instance.currentUser();
    DocumentSnapshot doc = await users.document(user.uid).get();
    var uid = doc['id'];
    //print(uid);
  ...
  {
    var url = "https://9dhzla746i.execute-api.ap-south-1.amazonaws.com/user/acceptconnection";
    var user = await FirebaseAuth.instance.currentUser();
    DocumentSnapshot doc = await users.document(user.uid).get();
    var uid = doc['id'];
    print(uid);
        ...
        children: <Widget>[
          Row(
            ...
            children: <Widget>[
              Expanded(
                child: ListTile(
                  ...
                  leading: GestureDetector(
                    onTap: ()=>showProfile(context, widget.user,widget.user.photoUrl,widget.curUser),
                    child: CircleAvatar(
                  ...
                  subtitle: Row(
                    children: <Widget>[
                      ...
                      Text("${widget.user.lollarAmount}",style: TextStyle(color: Colors.grey),),
                      Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 8),
                        child: Container(width: 1,height: 10,color: Colors.grey,),
                      ),
                      Container(
                        height: 15,
                        width: 15,
                      ...
                      Text("${widget.user.connection.length}",style: TextStyle(color: Colors.grey),),
                    ],
                  ),
                ),
              ),
              Row(
                children: <Widget>[
                  (widget.request && widget.accepted==false)?Padding(
